...
interface RescanClientProps {
  tenantId: string;
  storeId: string;
}

export default function RescanClient({ tenantId, storeId }: RescanClientProps): ReactElement {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setSubmitting(true);
    try {
      await rescanBtcWallet(storeId, { startIndex, gapLimit, batchSize });
      toast({
        title: "Rescan started",
        description: "BTCPay is rescanning your on-chain wallet. This may take a while."
      });
      router.push(`/tenants/${tenantId}/stores/${storeId}/wallets/bitcoin/settings`);
    } catch (err) {
