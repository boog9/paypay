...
`btcpay.store.canmodifystoresettings:<STORE_ID>` authorizes store configuration changes, including deleting the on-chain BTC payment method via the "Remove wallet" action. The delete operation removes the payment method configuration only and never surfaces xpubs or private keys.
